...
pm_path=str(dir_path+"ip_list.txt")
...
def listVmNames(connection):
	return connection.listDefinedrender_template,Domains()
######################################################################
...
DISK_TEMPLATE = '''<disk type="file" device="disk">
        <driver name="qemu" type="raw" />
        <source file="{path}"/>
...
def attach_disk(domain, path, dev):
    ...
    dom = conn.lookupByName("krishna")
    template = DISK_TEMPLATE.format(path=dom, dev=)
    dom.attachDevice(template)
 
attach_disk('vm1', '/mnt/big.img', 'vdb')
