...
######################################################################
def vmidExists(vmid):
	temp_path = dir_path + "allvmid.txt"
	ff=open(temp_path, 'r')
	for line in ff:
		if (vmid == line):
			return True
...
def add_vmid(vmid):
	...
	if vmid in id_array:
		...
		for i in id_array:
			f2 = open(dir_path + "allvmid.txt", 'w+')
			f2.write(i)
		f2.close()
		return 1
######################################################################
...
def removeVmidFromAllVmid(vmid):
	...
	for i in f:
		if str(i) != str(vmid):
			lis.append(i)
	f.close()
	f1 = open(dir_path + "allvmid.txt", 'w+')
	for i in lis:
		f1.write(i)
	f1.close()
######################################################################
	...
	if isVmNameExists(new_name):
		...
		for i in instance_types:
			...
			if str(i['tid']) == instance_type:
				...
				xml_string = get_xml(str(new_name), str(new_ram), str(new_cpu))
				pmid=""
				ip=""
...
def vmQuery():
	...
	# REMEMBER TO POPULATE PMID_IN< NAME_ID< INS_ID.txt to be refreshed and populated here or before any call
...
def vmDestroy():
	...
	if vmidExists(str(vmid)) :
		...
		if virtual_machine.destroy() == 0:
	 	...
	 	print "wrong id given; id not in file; false on vmidexists"
	 	return 'not okay'
	return "create over"
...
def pmQuery():
	...
	for i in f0:
		pmid_array.append(i.split('\n')[0])
	f0.close()
	print "pmid_array", pmid_array
	if this_pmid in pmid_array:
		print "440"
		...
		for i in f3:
			if this_pmid == i.split(':')[2].split(':')[0]:
				free.append(i.split(':')[0])
				free.append(i.split(':')[1])
				free.append(i.split(':')[2])
		
		print this_pmid, capacity, free, count
		return "(this_pmid+capacity+free+count)"
